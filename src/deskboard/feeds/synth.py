...
def demo_pnl_history(seed: int = 7, session_date: str = "2026-06-15", n_days: int = 500) -> pd.DataFrame:
    """Daily P&L history for the Health page: `n_days` business days ending the day before the
    session, a backtest expectation of $40 ± 500 a day (Sharpe ≈ 1.3) and a live series that
    tracks it with $150/day noise until the last 120 days, where it falls $120/day short —
    the kind of fade the CUSUM is there to catch. Fictional, seeded, regenerated by `deskboard record`."""
    rng = np.random.default_rng(seed + 1)
    end = pd.Timestamp(session_date) - pd.tseries.offsets.BDay(1)
    idx = pd.bdate_range(end=end, periods=n_days)
    bt = rng.normal(40.0, 500.0, n_days)
    live = bt + rng.normal(0.0, 150.0, n_days)
    live[-120:] -= 120.0
    return pd.DataFrame({"date": idx.strftime("%Y-%m-%d"), "pnl": live.round(2), "backtest": bt.round(2)})
